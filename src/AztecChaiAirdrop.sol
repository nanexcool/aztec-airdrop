...
contract AztecChaiAirdrop {
    ...
    function claim(bytes32[] calldata proof) external {
        ...
        require(done[msg.sender] == 0, "already claimed!");
        require(verify(proof, msg.sender), "address not in merkle tree!");
    ...
    function claimWithSig(bytes32[] calldata proof, address recipient, bytes calldata sig) external {
        ...
        require(done[who] == 0, "already claimed!");
        require(verify(proof, who), "address not in merkle tree!");

        done[who] = 1;

        require(chai.transfer(recipient, 1 ether), "could not transfer chai");
    }
